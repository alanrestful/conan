...
if (cluster.isMaster) {
    ...
    // 默认单线程跑
    var length = 6;
    var widthNum = Math.floor(Math.sqrt(length));
    var heightNum = length/widthNum;
    var sqrtWidth = 1280/heightNum;
    var sqrtHeight = 773/widthNum;

    console.log("widthNum", widthNum, "sqrtWidth", sqrtWidth, "sqrtHeight", sqrtHeight);
    for (var i = 0; i < length; i++) {
        var wk = cluster.fork();
        var heightLoc = Math.floor(i/heightNum)*sqrtHeight;
        wk.send([i%heightNum*sqrtWidth , heightLoc > 0 ? heightLoc+21 : heightLoc, sqrtWidth, sqrtHeight]);
    }
    ...
    process.on('message', function(msg) {
		...
		driver.manage().window().setSize(msg[2], msg[3]);
